...
(defun make-point (x y)
  ...
  (c-let ((point sdl3-ffi:sdl-point))
    (setf (point :x) x
          (point :y) y)
    point))

(define-struct-accessors (point sdl3-ffi:sdl-point)
  :x :y)

(defun make-f-point (x y)
  "Return an SDL_FPoint filled in with the arguments."
  (c-let ((point sdl3-ffi:sdl-f-point))
    (setf (point :x) x
          (point :y) y)
    point))

(define-struct-accessors (f-point sdl3-ffi:sdl-f-point)
  :x :y)

(defmacro c-point ((wrapper-var) &body body)
  `(c-let ((,wrapper-var sdl3-ffi:sdl-point :from ,wrapper-var))
     ,@body))

(defmacro c-f-point ((wrapper-var) &body body)
  `(c-let ((,wrapper-var sdl3-ffi:sdl-f-point :from ,wrapper-var))
     ,@body))
...
(defun points* (&rest points)
  ...
  (let ((num-points (length points)))
    (c-let ((c-points sdl3-ffi:sdl-point :count num-points))
      (loop :for i :from 0
            :for point :in points
...
(defun f-points* (&rest points)
  ...
  (let ((num-points (length points)))
    (c-let ((c-points sdl3-ffi:sdl-f-point :count num-points))
      (loop :for i :from 0
            :for point :in points
            :do (copy-into-point (c-points i) point))
      (values (c-points &) num-points))))
...
(defmacro c-rect ((r) &body body)
  `(c-let ((,r sdl3-ffi:sdl-rect :from ,r))
     ,@body))
...
(define-struct-accessors (rect sdl3-ffi:sdl-rect)
  :x :y (width :w) (height :h))

(defmethod print-object ((rect sdl3-ffi:sdl-rect) stream)
  (c-rect (rect)
...
(defmacro let-rects (bindings &body body)
  (flet ((make-rect-list (bindings)
           (loop :for rect :in bindings :collect (list rect 'sdl3-ffi:sdl-rect))))
...
(defun rects* (&rest rects)
  ...
  (let ((num-rects (length rects)))
    (c-let ((c-rects sdl3-ffi:sdl-rect :count num-rects))
      (loop :for i :from 0
            :for rect :in rects
...
(defun has-rect-intersection (first-rect &rest rects)
...
structure."
  (loop :for (a b) :in (unique-pairs `(,first-rect ,@rects))
        :do (unless (sdl-true-p (sdl-has-rect-intersection-float a b))
              (return-from has-rect-intersection-float-p nil)))
  t)

(defun rect-and-line-intersection (rect x1 y1 x2 y2)
...
coordinates of the line clipped to the boundary of the rectangle."
  (c-with ((x1pos :int)
    ...
    (setf x1pos x1
          ...
          x2pos x2
          y2pos y2)
    (let ((intersected (sdl-true-p (sdl-get-rect-and-line-intersection rect (x1pos &) (y1pos &) (x2pos &) (y2pos &)))))
      (values intersected x1pos y1pos x2pos y2pos))))

(defun rect-and-line-intersection-float (rect x1 y1 x2 y2)
  "Returns five values where the first value is T if the coordinates of the line
 intersect RECT. The remaining returned values represent the starting and ending
coordinates of the line clipped to the boundary of the rectangle."
  (c-with ((x1pos :float)
    ...
    (setf x1pos x1
          ...
          x2pos x2
          y2pos y2)
    (let ((intersected (sdl-true-p (sdl-get-rect-and-line-intersection-float rect (x1pos &) (y1pos &) (x2pos &) (y2pos &)))))
      (values intersected x1pos y1pos x2pos y2pos))))

(defun rect-union (first-rect &rest rects)
...
rectangle as a newly allocated SDL_Rect in which all others fit perfectly."
  (let ((union-rect (copy-rect first-rect)))
    (dolist (rect rects)
      (sdl-get-rect-union rect union-rect union-rect))
    union-rect))

;;; SDL_RectF

(defmacro c-f-rect ((r) &body body)
  `(c-let ((,r sdl3-ffi:sdl-f-rect :from ,r))
     ,@body))
...
(declaim (ftype (function (float float float float) sdl3-ffi:sdl-f-rect) make-f-rect))
(defun make-f-rect (x y w h)
  (c-let ((f-rect sdl3-ffi:sdl-f-rect))
    (setf (f-rect :x) x
          (f-rect :y) y
          (f-rect :w) w
          (f-rect :h) h)
...
(define-struct-accessors (f-rect sdl3-ffi:sdl-f-rect)
  :x :y (width :w) (height :h))

(defmethod print-object ((f-rect sdl3-ffi:sdl-f-rect) stream)
  (c-f-rect (f-rect)
...
(defmacro let-f-rects (bindings &body body)
  (flet ((make-f-rect-list (bindings)
           (loop :for f-rect :in bindings :collect (list f-rect 'sdl3-ffi:sdl-f-rect))))
...
(defun f-rects* (&rest f-rects)
  ...
  (let ((num-f-rects (length f-rects)))
    (c-let ((c-f-rects sdl3-ffi:sdl-f-rect :count num-f-rects))
      (loop :for i :from 0
