...
(defvar *user-event-id*
  (c-let ((new-atomic sdl3-ffi:sdl-atomic-int))
    (setf (new-atomic :value) 0)
    new-atomic))
(defvar *event-loop* nil)

(defun new-event (&optional (event-type :first))
  (c-let ((event sdl3-ffi:sdl-event))
...
(defun register-user-event-type (user-event-type)
  ...
  (multiple-value-bind (event-type-code event-type-found)
    ...
    (if event-type-found
        event-type-code
        (let ((new-event-code (sdl3-ffi.functions::sdl-register-events 1)))
...
(defmacro with-sdl-event ((event &optional (event-type :first)) &body body)
  "Allocate and automatically free an sdl event struct."
  `(c-let ((,event sdl3-ffi:sdl-event :from (new-event ,event-type)))
...
(defun get-event-code (event-type)
  (multiple-value-bind (user-event-code is-user-event) (gethash event-type *user-event-codes*)
    (cond
       ...
       *lisp-message-event*)
      (t
       (enum-value 'sdl3-ffi:sdl-event-type event-type)))))

(defun get-event-type (event)
  (c-let ((event sdl3-ffi:sdl-event :from event))
    (multiple-value-bind (user-event-type is-user-event) (gethash (event :type) *user-event-types*)
      (cond
        ...
        ((eq (event :type) *lisp-message-event*)
         :lisp-message)
        (t
         (or (enum-key 'sdl3-ffi:sdl-event-type (event :type))
             (event :type)))))))
...
Stores the optional user-data in sdl3::*user-events*"
...
(defparameter *event-type-to-accessor*
  '((:camera-device-added . :cdevice)
    (:camera-device-approved . :cdevice)
    (:camera-device-denied . :cdevice)
    (:camera-device-removed . :cdevice)
    (:gamepad-axis-motion . :gaxis)
    (:gamepad-button-down . :gbutton)
    (:gamepad-button-up . :gbutton)
    (:gamepad-added . :gdevice)
    (:gamepad-remapped . :gdevice)
    (:gamepad-removed . :gdevice)
    (:dropfile . :drop)
    (:finger-motion . :tfinger)
    (:finger-down . :tfinger)
    (:finger-up . :tfinger)
    (:joystick-axis-motion . :jaxis)
    (:joystick-ball-motion . :jball)
    (:joystick-button-down . :jbutton)
    (:joystick-button-up . :jbutton)
    (:joystick-added . :jdevice)
    (:joystick-removed . :jdevice)
    (:joystick-hat-motion . :jhat)
    (:key-down . :key)
    (:key-up . :key)
  ...
  (mapcar (lambda (param)
            (let ((keyword (first param))
              ...
              (if (eql keyword :user-data)
                  `(,binding (get-user-data (c-ref ,event-var sdl3-ffi:sdl-event ,ref :code)))
                  (if (and (or (eql ref :text) (eql ref :edit)) (eql keyword :text))
                      `(,binding (c-ref ,event-var sdl3-ffi:sdl-event ,ref ,keyword string))
                      `(,binding (c-ref ,event-var sdl3-ffi:sdl-event ,ref ,keyword))))))
          params))
