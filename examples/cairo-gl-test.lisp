...
(defun cairo-test (&key (width *texture-size*) (height *texture-size*)
  ...
  (sdl3:with-init (:everything)
    (multiple-value-bind (window renderer)
        (sdl3:create-window-and-renderer "Cairo Test" width height)
      (sdl3:with-gl-context (gl window)
        (sdl3:gl-make-current window gl)
        ...
        (autowrap:with-alloc (tex :unsigned-char (* *texture-size* *texture-size* 4))
          (let* ((program (gl-init-shaders))
            ...
            ;; tex-sub-image later. Also, the actual internal format is bgra, so
            ;; it matches what SDL is going to say about the surface we're going
            ;; to create.
            ...
            (sdl3:with-event-loop (:method :poll)
              (:keyup () (sdl3:push-event :quit))
              (:mousebuttondown () (sdl3:push-event :quit))
